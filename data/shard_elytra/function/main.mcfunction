...
# execute as @a if items entity @s[tag=!beacon, scores={flying=1.., usedRocket=1..}, nbt={Dimension:"minecraft:overworld"}] armor.chest elytra run tag @s add removeElytraWarning
execute as @a if items entity @s[tag=!beacon, scores={flying=1.., usedRocket=1..}, nbt={Dimension:"minecraft:overworld"}] armor.chest elytra run scoreboard players add @s elytraWarning 1
...
tellraw @a[scores={elytraWarning=1}] {"text":"Warning: You can only rockets to boost your elytra in the overworld if under the effect of a beacon.", "color": "yellow"}
